! function(t) {
	...
	function e(t, n) {
		...
		this._maxEntries = Math.max(4, t || 9), this._minEntries = Math.max(2, Math.ceil(.4 * this._maxEntries)), n && this
			._initFormat(n), this.clear()
	}
	...
	function h(t, n) {
		return t.minX = Math.min(t.minX, n.minX), t.minY = Math.min(t.minY, n.minY), t.maxX = Math.max(t.maxX, n.maxX), t.maxY =
			Math.max(t.maxY, n.maxY), t
	}
	...
	function _(t, n, i, e, r) {
		for (var o, s = [n, i]; s.length;)(i = s.pop()) - (n = s.pop()) <= e || (o = n + Math.ceil((i - n) / e / 2) * e, a(
			t, o, n, i, r), s.push(n, o, o, i))
	}
	e.prototype = {
		all: function() {
		...
		search: function(t) {
			...
			if (!d(t, n)) return i;
			for (var e, r, o, s, h = []; n;) {
				for (e = 0, r = n.children.length; e < r; e++) o = n.children[e], d(t, s = n.leaf ? a(o) : o) && (n.leaf ? i.push(
					o) : f(t, s) ? this._all(o, i) : h.push(o));
				n = h.pop()
			}
			return i
		},
		...
		remove: function(t, n) {
			...
			for (var i, a, e, o, s = this.data, h = this.toBBox(t), l = [], c = []; s || l.length;) {
				if (s || (s = l.pop(), a = l[l.length - 1], i = c.pop(), o = !0), s.leaf && -1 !== (e = r(t, s.children, n)))
					return s.children.splice(e, 1), l.push(s), this._condense(l), this;
				o || s.leaf || !f(s, h) ? a ? (i++, s = a.children[i], o = !1) : s = null : (l.push(s), c.push(i), i = 0, a = s,
					s = s.children[0])
			}
			return this
		},
		...
		_build: function(t, n, i, a) {
			...
			a || (a = Math.ceil(Math.log(r) / Math.log(s)), s = Math.ceil(r / Math.pow(s, a - 1))), (e = p([])).leaf = !1, e
				.height = a;
			var h, l, c, u, m = Math.ceil(r / s),
				f = m * Math.ceil(Math.sqrt(s));
			for (_(t, n, i, f, this.compareMinX), h = n; h <= i; h += f)
				for (_(t, h, c = Math.min(h + f - 1, i), m, this.compareMinY), l = h; l <= c; l += m) u = Math.min(l + m - 1, c),
		...
		_chooseSubtree: function(t, n, i, a) {
			for (var e, r, o, s, h, l, c, m, f, d; a.push(n), !n.leaf && a.length - 1 !== i;) {
				for (c = m = 1 / 0, e = 0, r = n.children.length; e < r; e++) h = u(o = n.children[e]), f = t, d = o, (l = (
						Math.max(d.maxX, f.maxX) - Math.min(d.minX, f.minX)) * (Math.max(d.maxY, f.maxY) - Math.min(d.minY, f.minY)) -
					h) < m ? (m = l, c = h < c ? h : c, s = o) : l === m && h < c && (c = h, s = o);
				n = s || n.children[0]
			}
			return n
		},
		...
		_split: function(t, n) {
			...
			s.height = i.height, s.leaf = i.leaf, o(i, this.toBBox), o(s, this.toBBox), n ? t[n - 1].children.push(s) : this
				._splitRoot(i, s)
		},
		...
		_chooseSplitIndex: function(t, n, i) {
			...
			for (l = c = 1 / 0, a = n; a <= i - n; a++) e = s(t, 0, a, this.toBBox), r = s(t, a, i, this.toBBox), f = e, d =
				r, void 0, void 0, void 0, void 0, p = Math.max(f.minX, d.minX), _ = Math.max(f.minY, d.minY), g = Math.min(f.maxX,
					d.maxX), x = Math.min(f.maxY, d.maxY), o = Math.max(0, g - p) * Math.max(0, x - _), h = u(e) + u(r), o < l ? (
					l = o, m = a, c = h < c ? h : c) : o === l && h < c && (c = h, m = a);
			return m
		},
		...
		_condense: function(t) {
			for (var n, i = t.length - 1; i >= 0; i--) 0 === t[i].children.length ? i > 0 ? (n = t[i - 1].children).splice(n
				.indexOf(t[i]), 1) : this.clear() : o(t[i], this.toBBox)
		},
		_initFormat: function(t) {
			var n = ["return a", " - b", ";"];
			this.compareMinX = new Function("a", "b", n.join(t[0])), this.compareMinY = new Function("a", "b", n.join(t[1])),
	...
	t.exports = function(t) {
		var n = (t.Layer ? t.Layer : t.Class).extend({
			...
			onAdd: function(t) {
				this._map = t, this._canvas || this._initCanvas(), this.options.pane ? this.getPane().appendChild(this._canvas) :
					t._panes.overlayPane.appendChild(this._canvas), t.on("moveend", this._reset, this), t.on("resize", this._reset,
						this), t.on("click", this._executeListeners, this), t.on("mousemove", this._executeListeners, this)
			},
			onRemove: function(t) {
			...
			_addMarker: function(n, i, a) {
				n._map = this._map, this._markers || (this._markers = new rbush), this._latlngMarkers || (this._latlngMarkers =
					new rbush, this._latlngMarkers.dirty = 0, this._latlngMarkers.total = 0), t.Util.stamp(n);
			...
			_drawMarker: function(t, n) {
				...
				else if (i._imageLookup[a]) t.canvas_img = i._imageLookup[a][0], !1 === i._imageLookup[a][1] ? i._imageLookup[
					a][2].push([t, n]) : i._drawImage(t, n);
				else {
					var e = new Image;
			...
			_drawImage: function(t, n) {
				...
				this._context.drawImage(t.canvas_img, n.x - i.iconAnchor[0], n.y - i.iconAnchor[1], i.iconSize[0], i.iconSize[
					1])
			},
			_reset: function() {
			...
			_executeListeners: function(t) {
				...
				e && e.length > 0 ? (n._map._container.style.cursor = "pointer", "click" === t.type && (e[0].data.getPopup() &&
					e[0].data.openPopup(), n._onClickListeners.forEach(function(n) {
						n(t, e)
					})), "mousemove" === t.type && (e[0].data.getTooltip() && (n._openToolTip = e[0].data, e[0].data.openTooltip()),
